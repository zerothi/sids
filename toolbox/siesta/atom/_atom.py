...
class AtomInput:
    ...
    def from_input(cls, inp):
        ...
        if isinstance(inp, (tuple, list)):
            ...
            # it is either a long string, or a file
            if Path(inp).is_file():
                inp = open(inp, 'r').readlines()
            else:
                inp = inp.splitlines()
        else:
            raise ValueError(f"Unknown input format `inp`?")
    ...
    def plot(self, path=None, plot=('wavefunction', 'charge', 'log', 'pseudopotential'),
             l='spdf'):
        ...
        for this_plot in plot:
            ...
            elif this_plot == "pseudopotential":
                plot_pseudopotential(axs[ir][ic])
